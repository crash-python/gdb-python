...
static pytarget_object * target_ops_to_target_obj (struct target_ops *ops)
{
    ...
    /* Native target exported to Python */
				...
				struct thread_info *info)
{
    ...
    gdb::unique_xmalloc_ptr<char> name = python_string_to_host_string (
								result.get ());
    if (name == NULL)
      return NULL;
			   ...
			   ULONGEST offset, ULONGEST len, ULONGEST *xfered_len)
{
    ...
    if (gdb_readbuf)
      {
	bytes = PyByteArray_FromStringAndSize ((char *) gdb_readbuf, len);
	readbuf.reset (bytes);
	if (readbuf == NULL)
	  return rt;
      }
    ...
    if (gdb_writebuf)
      {
	bytes = PyByteArray_FromStringAndSize ((char *) gdb_writebuf, len);
	writebuf.reset (bytes);
	if (writebuf == NULL)
	  return rt;
      }
    else
...
tgtpy_default_to_thread_name (PyObject *self, PyObject *args)
{
  ...
  return PyString_FromString ("NoThreadName");
}

static PyMethodDef pytarget_object_methods[] =
{
  { "to_thread_name_int", tgtpy_default_to_thread_name,
    METH_VARARGS | METH_KEYWORDS, "to_thread_name (thread_info) -> String.\n\
